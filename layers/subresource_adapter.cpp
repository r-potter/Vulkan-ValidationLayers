...
IndexType BlitRangeEncoder::Decode1D(const IndexType& encode, BlitSubresource& blit_decode) const {
    blit_decode.offset.y = 1;
    blit_decode.offset.x = static_cast<int32_t>(encode / OffsetXSize());
    return (encode % OffsetXSize());
}

IndexType BlitRangeEncoder::Decode2D(const IndexType& encode, BlitSubresource& blit_decode) const {
...
static bool IsValid(const BlitRangeEncoder& encoder, const VkImageSubresourceRange& bounds, const VkOffset2D& offset,
...
BlitRangeGenerator& BlitRangeGenerator::operator++() {
    mip_index_++;
    // NOTE: If all selected mip levels are done at once, mip_count_ is set to one, not the number of selected mip_levels
    if (mip_index_ >= mip_count_) {
        const auto last_aspect_index = aspect_index_;
